...
const Notification = ({ navigation }) => {
    ...
    const getDateinFormat = (notificationTime) => {
        // let date = getTimezoneMomentDate(notificationTime, defaultSocietyInfo.timezone)
        let days = moment().diff(moment(notificationTime), 'days');
        if (days == 0) {
            let hours = moment().diff(moment(notificationTime), 'hours');
            if (hours == 0) {
                let minutes = moment().diff(moment(notificationTime), 'minutes')
                if (minutes == 0) {
        ...
        if (days >= 7) {
            let weeks = moment().diff(moment(notificationTime), 'weeks')
            if (weeks == 1) {
                return `a week ago`;
            } else {
        ...
        if (days >= 31) {
            let months = moment().diff(moment(notificationTime), 'months')
            if (months == 1) {
                return `a month ago`;
            } else {
        ...
        if (days >= 365) {
            let years = moment().diff(moment(notificationTime), 'years')
            if (years == 1) {
                return `a year ago`;
            } else {
                ...
                <View style={styles.notificationMainView}>
                    ...
                    <View style={styles.notificationRightMainView}>
                        <View style={styles.stateViewStyle}>
                            <Text style={styles.timeTextStyle}>{getDateinFormat(item.notificationTime)}</Text>
